...
async function launchBrowser() {
  ...
  // Lenovo's CDN performs TLS fingerprinting (JA3/JA4) that rejects the
  // headless shell's ClientHello.  Chrome's "new headless" mode produces
  // the same TLS fingerprint as headed Chrome, bypassing detection.
...
async function getMachineSpecs(serialNumber, existingPage) {
  ...
  try {
    ...
    // Use JavaScript to click the spec info button (more reliable than selector waiting)
